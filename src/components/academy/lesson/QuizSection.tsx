...
export function QuizSection({ quiz, lessonSlug }: QuizSectionProps) {
  ...
  const { completeLesson, saveQuizResult, isLessonComplete } = useProgress()
  ...
  if (alreadyComplete && !finished) {
    const stored = useProgress.getState().quizResults[lessonSlug]
    return (
      <Card className="border-editorial-green/30 bg-editorial-green-soft/20">
        <CardContent className="p-5 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-sm text-editorial-green">Knowledge Check Complete</p>
            {stored && (
        ...
        <Card className="border-editorial-green/30 bg-editorial-green-soft/20">
          <CardContent className="p-6 text-center space-y-3">
            ...
            <p className="text-xl font-bold font-serif">
              {score === quiz.length ? "Perfect Score!" : score >= quiz.length - 1 ? "Great Work!" : "Keep Learning!"}
            </p>
